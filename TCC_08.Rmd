...
\end{equation*}
onde os parâmetros $\beta_0, \beta_1,\ldots,\beta_p$ são os coeficientes de regressão e o termo $\varepsilon$ é um erro aleatório que deve satisfazer os pressupostos de ter média zero, variância $\sigma^2$ constante e  serem não correlacionados.  O parâmetro $\beta_j$  representa a mudança esperada na resposta $y$ a cada mudança unitária  de $x_j$ quando todas as outras variáveis regressoras são constantes e, por isso,  são frequentemente chamados de coeficientes parciais de regressão.  Em geral, os coeficientes de regressão são parâmetros desconhecidos e podem ser estimados utilizando, dentre outras técnicas, o método do mínimos quadrados, por exemplo.
