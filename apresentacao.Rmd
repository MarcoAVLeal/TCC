...
* As funções $f_j$ devem ser estimadas por meio de **suavizadores**, que estimam uma tendência menos variável e descreve sua dependência em relação à variável resposta.
...
\item Em particular, serão abordados os suavizadores \textit{Kernel}, \textit{Loess} e \textit{Splines} de regressão considerando apenas uma covariável, e  seus desempenhos  serão comparados com foco principal na predição.
...
\item Apresentar algumas técnicas de suavização  para estimar funções não paramétricas presentes nos modelos aditivos;
...
* No caso de uma variável preditora é chamado de suavizador em diagrama de dispersão.
...
* O Suavizador mais simples é o caso dos preditores **categóricos**;

* Para suavizar $y$ podemos simplesmente **realizar as médias** de seus valores cada categoria.

* Este processo captura a **tendência** de $y$ em $x$. 

*  Em relação as médias realizadas nas vizinhanças em torno do valor alvo, têm-se duas decisões a serem tomadas: 
...
* A questão de como realizar a média em uma vizinhança é a questão de qual **tipo de suavizador a utilizar**;
...
* Suavizador Loess, também chamado de \textit{Lowess}, essa técnica pode ser vista como uma linha móvel com pesos locais; 

* Os pesos $w_i$ são designados para cada ponto contidas nos vizinhos mais próximos, usando a função de peso tri-cúbica:
